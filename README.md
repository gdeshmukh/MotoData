...
- **Delta-time** — a Δt B−A panel shows time gained/lost against reference lap A.
  It only means anything against distance, so the Δ button flips the x-axis to
  distance when you turn it on, and greys out when there is nothing to compare.
...
.venv\Scripts\python viewer.py "C:\path\to\Data"
```
...
live in `~/.motodata/`. Telemetry archives and their sibling XML files are opened
read-only.
...
conservative unit inference. Descriptions are not authoritative unit metadata;
ambiguous units remain unknown unless set in `channel_units_overrides.json`.
...
with LapData(info.ztx, info.lap_time, lap_distance=info.distance) as lap:
...
engineering units. A channel's sample rate is `samples / lap_time`; nominal rates
are matched from 0.5–1000 Hz. Because `.sar` files have no sample timestamps, a
short sample count is distributed across the lap duration. Lap metadata lives in
sibling XML files.
